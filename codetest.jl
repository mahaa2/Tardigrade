...
# define priors
pGauss = priorGauss(0.0, 100.0, priorEmpty(), priorEmpty());

# define models
likA = likEvA(0.0, 0.0, pGauss, pGauss, -3.91)
...
ka = pcubature(x -> exp(likA.llik(likA, y, x)), dA.μ-6.*Δa, dA.μ+6.*Δa)[1]
kh = pcubature(x -> exp(likH.llik(likH, y, x)), dH.μ-6.*Δh, dH.μ+6.*Δh)[1]
k1 = pcubature(x -> exp(lik1.llik(lik1, y, x)), d1.μ-6.*Δ1, d1.μ+6.*Δ1)[1]
...
zsea = Float64[pdf(EA, [x1; x2]) for x1 in xsa, x2 in ysa]; # approximate Gaussian (elliptical) countours
...
zseh = Float64[pdf(EH, [x1; x2]) for x1 in xsh, x2 in ysh]; # approximate Gaussian (elliptical) countours
...
zse1 = Float64[pdf(E1, [x1; x2]) for x1 in xsh, x2 in ysh]; # approximate Gaussian (elliptical) countours
...
# EP
colse = repeat(["black"; "blue"], inner = length(lvl))

pa = plot([layer(x = cae[i][:, 1], y = cae[i][:, 2], Geom.path, 
           Theme(default_color = color(colse[i]))) for i in 1:length(cal)] ...,
           Theme(background_color = color("White")), Guide.yticks(label=false),
           Guide.xticks(label=false), Guide.ylabel(nothing), Guide.xlabel("Constant volume/area element (Achcar)"),
           Coord.Cartesian(xmin=dA.μ[1]-3.*Δa[1], xmax=dA.μ[1]+3.*Δa[1],
           ymin=dA.μ[2]-3.*Δa[2], ymax=dA.μ[2]+3.*Δa[2]));

ph = plot([layer(x = che[i][:, 1], y = che[i][:, 2], Geom.path, 
           Theme(default_color = color(colse[i]))) for i in 1:length(chl)] ...,
           Theme(background_color = color("White")), Guide.yticks(label=false),
           Guide.xticks(label=false), Guide.ylabel(nothing), Guide.xlabel("Orthogonal (Jeffreys)"),
           Coord.Cartesian(xmin=dH.μ[1]-1.1.*Δh[2], xmax=dH.μ[1]+1.1.*Δh[2],
           ymin=dH.μ[2]-3.*Δh[2], ymax=dH.μ[2]+3.*Δh[2]));

p1 = plot([layer(x = c1e[i][:, 1], y = c1e[i][:, 2], Geom.path, 
           Theme(default_color = color(colse[i]))) for i in 1:length(c1l)] ...,
           Theme(background_color = color("White")), Guide.yticks(label=false),
           Guide.xticks(label=false), Guide.ylabel(nothing), Guide.xlabel("Real line"),
           Coord.Cartesian(xmin=d1.μ[1]-0.9.*Δ1[2], xmax=d1.μ[1]+0.9.*Δ1[2],
           ymin=d1.μ[2]-3.*Δ1[2], ymax=d1.μ[2]+3.*Δ1[2]));

myplot2 = vstack(hstack(p1, ph), hstack(pa, pn))
draw(PNG("myplot2.png", 8inch, 8inch, dpi = 100), myplot2)
